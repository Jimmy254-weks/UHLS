...
.appointment-table .center {
    text-align: center;
}

/* Style for the 'visible' class */
.appointment-table .visible {
    display: block;
}
...
<div class="container">
    <div class="col">
        <h1 class="title">Patients | Appointment History</h1>
    </div>
    <div class="row">
        <div class="col-md-12">
            <p style="color:red;"><?php echo htmlentities($_SESSION['msg']);?> <!-- Displaying the message stored in the session variable 'msg', HTML entity encoded -->
            <?php echo htmlentities($_SESSION['msg']="");?></p>	<!-- Resetting the value of 'msg' session variable to an empty string after displaying it, HTML entity encoded -->
...
<?php
    ...
    $sql = mysqli_query($conn, "SELECT doctors.doctorName as docname, users.fullName as pname, appointment.*  FROM appointment JOIN doctors ON doctors.id = appointment.doctorId JOIN users ON users.id = appointment.userId");

    // Counter variable initialization.  number the rows sequentially starting with 1
    $cnt = 1;
...
?>
    <tr>
        ...
        <!-- Hidden columns for doctor name and patient name -->
        <td class="hidden"><?php echo $row['docname'];?></td>
        <td class="hidden"><?php echo $row['pname'];?></td>
        
        <!-- Displaying doctor specialization, consultation fees, appointment date and time, posting date -->
        <td><?php echo $row['doctorSpecialization'];?></td>
        <td><?php echo $row['consultancyFees'];?></td>
        <td><?php echo $row['appointmentDate'];?> / <?php echo $row['appointmentTime'];?></td>
        <td><?php echo $row['postingDate'];?></td>
        
        <!-- Displaying appointment status based on user and doctor status -->
        <td>
            <?php 
        ...
        <!-- Displaying further action based on user and doctor status -->
        <td>
            <div class="visible">
                <?php 
                if (($row['userStatus'] == 1) && ($row['doctorStatus'] == 1)) {
...
<?php 
        // Incrementing the counter ensuring that each row is numbered sequentially starting from 1
        $cnt = $cnt + 1;
    }
?>
</tbody>
...
<footer>
    <div class="footer">
        <p>Copyright &copy; <?php echo date("Y"); ?>  UHL | Designed by James Wekesa & Jo-Anne Khang</p>
    </div>
</footer>
</body>
</html>
